        ...
        string? defaultEdition = null)
    {
        ...
        [
            ...
            CreateTool(
                ...
                "plan_gp_job_sizing",
                "Compute the runtime SIZING HINT for a single geoprocessing (GP) job: select the durable job-definition tier (s/m/l/xl from ephemeralStorageGib <=20/<=50/<=100/<=200) from the per-env substrate's pre-provisioned pool and produce the AWS Batch SubmitJob overrides (loose batch.* params batch.vcpus/batch.memory_mib/batch.timeout_seconds/batch.retry_attempts/batch.ephemeral_gib) the server applies at runtime. PURE PLANNING AID: NO terraform, NO infra, NO deploy-control operation recorded — per-job sizing is a SubmitJob-time concern with zero infra change. ephemeralStorageGib above 200 exceeds the Fargate ceiling / tier pool and is an error; gpuCount>0 is an advisory note (the default Fargate-Spot substrate has no GPU tiers), not a hard reject. Returns the selected tier token, the gp_job_definition_arns.<tier> output path the server resolves, and the SubmitJob override map."),
            CreateTool(
                (string service, string environmentsCsv, string poolVmSize, string image, string acrName, int maxNodes, bool createWorkerGdalAcr, string owner)
                    => consoleBridge.PlanAzureGpSubstrateAsync(service, environmentsCsv, poolVmSize, image, acrName, maxNodes, createWorkerGdalAcr, owner),
                "plan_azure_gp_substrate",
                "Plan provisioning/updating the durable PER-ENVIRONMENT AZURE geoprocessing (GP) substrate (Azure Batch account, a SINGLE worker pool, user-assigned task identity, ACR holding the GDAL worker image, and the output blob container), recording a PLAN-FIRST, advisory deploy-control proposal (submitImmediately=false; never executes). The Azure equivalent of plan_gp_substrate. Provisioned RARELY (when GP capability is added/updated in an env), NOT per job. UNLIKE AWS, Azure sizes per-POOL and the MVP is a SINGLE pool, so there is NO tier selector. Per-env config: poolVmSize (e.g. Standard_D4s_v5, shared by the pool), maxNodes (pool autoscale ceiling; 0=>8), image (empty=ACR default), acrName, createWorkerGdalAcr. Binds to substrate OUTPUTS the server consumes (gp_batch_account_url, gp_pool_id, gp_batch_account_id, gp_task_identity_id, gp_task_identity_principal_id, gp_acr_login_server, gp_output_container_url, gp_control_plane_backend_name=honua-azure-batch) — never input-variable names. Per-job sizing (task timeout/retry/image) is a task-add-time runtime concern with zero infra change; use plan_azure_gp_job_sizing for that. The real terraform apply stays behind the agent's execution/approval gates exactly like the other runtime adapters."),
            CreateTool(
                (int taskTimeoutMinutes, int maxTaskRetryCount, string containerImage, string poolId)
                    => consoleBridge.PlanAzureGpJobSizingAsync(taskTimeoutMinutes, maxTaskRetryCount, containerImage, poolId),
                "plan_azure_gp_job_sizing",
                "Compute the runtime SIZING HINT for a single AZURE geoprocessing (GP) job. The Azure equivalent of plan_gp_job_sizing, but SIMPLER: Azure sizes per-POOL and the MVP is a SINGLE pool, so there is NO tier selection and NO vCPU/memory override (those are fixed by the provisioned pool). Sets the constant azure.batch.pool_id (resolve from the substrate output gp_pool_id; pass poolId to embed it) plus the narrow per-task overrides Azure Batch supports: azure.batch.task_timeout_minutes, azure.batch.max_task_retry_count, and optionally azure.batch.container_image. PURE PLANNING AID: NO terraform, NO infra, NO deploy-control operation recorded — per-job sizing is a task-add-time concern with zero infra change. task_timeout_minutes<=0 or max_task_retry_count outside 0..100 is an error. Returns the azure.batch.* override map the server applies at task-add time."),
            CreateTool(
                (string operationId)
