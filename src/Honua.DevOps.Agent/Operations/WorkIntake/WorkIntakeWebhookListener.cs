...
/// <summary>
/// Lightweight HTTP listener for the work-intake webhook. Uses BCL-only
/// <see cref="HttpListener"/> so honua-devops stays on the plain console SDK
/// with zero added dependencies — a direct sibling of
/// <see cref="EscalationWebhookListener"/>. Binds to localhost only; front it
/// with a tunnel/proxy for remote delivery.
/// </summary>
internal sealed class WorkIntakeWebhookListener : IAsyncDisposable
{
    ...
    internal async Task RunAsync(CancellationToken cancellationToken)
    {
        _httpListener.Start();
        _stdout.WriteLine(
            $"honua-devops work-intake listener bound to {EndpointUri} (provider={_configuration.Provider.ToString().ToLowerInvariant()}, auto-draft={_configuration.AutoDraft.ToString().ToLowerInvariant()}).");
        _stdout.WriteLine("Awaiting signed work-intake webhooks. Press Ctrl+C to exit.");
    ...
    private async Task HandleContextAsync(HttpListenerContext context, CancellationToken cancellationToken)
    {
        try
        {
            ...
            string? signatureHeader = context.Request.Headers[WorkIntakeWebhookPayload.SignatureHeader];
            ...
            {
                _stderr.WriteLine($"work-intake rejected: status={result.StatusCode} reason={result.Reason}");
            }
        ...
        {
            _stderr.WriteLine($"work-intake listener I/O error: {exception.Message}");
        }
    }
