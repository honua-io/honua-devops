...
- Bounded deployment recovery (protected deployments only): a qualified target may additionally carry a pre-authorized, deterministic compensation scoped to the exact prior/candidate revision pair approved at deploy time. This is NOT a tool you call — it has no model-facing entry point and fires only when its own trigger (the deployment's safety policy) does, gated independently by `HONUA_DEVOPS_PROTECTED_RECOVERY_ENABLED`. If asked about it, describe the bound scope and refer to `find_recent_operations` to observe the outcome; never suggest invoking it directly.
