...
**Status:** execution tracker — backlog filed and agentflow execution in progress
**Date:** 2026-05-21
...
**Execution update:** 2026-05-23 — the QGIS plugin source repo and static landing page are live. GitHub Release ZIP, QGIS Plugin Repository approval, screenshots, and demo video remain release-owner follow-ups.
...
9. [Decision points needing founder sign-off](#9-decision-points-needing-founder-sign-off)
10. [Open questions](#10-open-questions)
11. [Appendix A — Top issues by repo](#appendix-a--top-issues-by-repo)
...
2. **The current backlog over-indexes on enterprise plumbing and under-indexes on the differentiated narrative.** 33 open issues on honua-server, of which 15 are P3/P4/GA enterprise wishlist items (SAML, SCIM, SIEM, RBAC, multi-tenancy, plugin SDK, federated queries) — defensible deferral, but it crowds out the items that would actually create market pull. At scan time, zero open issues mentioned the open-weights GIS LLM, a QGIS plugin, NIM integration, GovCloud, or specific SLG customer segments. FedRAMP / SOC 2 sits at P4/GA which is incompatible with the SLG GTM thesis.
...
**The 60-day plan:** unblock the cloud-staging chain (week 1-2), ship the Esri migration proof package (week 3-6), land the marketplace path (week 7-9), and convert the first pilot while publishing open AI assets (week 10-12). File five new strategic-gap epics this week so they appear in the backlog and don't get forgotten. Promote FedRAMP/SOC 2 to a real epic now.

**The biggest single ask in this plan:** treat **honua-server #965 (TLS cert misconfig)** as P0/stop-the-line. It is currently blocking SDK staging smoke, terraform live validation, and the proof chain that supports both Inception and the marketplace launch.

---

## 2. Where the portfolio actually stands

**15 active repos, 102 open issues, zero stale (>6 months), zero customer-reported, zero P0 labels in use.**
...
This scorecard answers: *for each strategic priority we've articulated, does the current backlog actually execute on it?*
...
| **Esri migration tooling (paid)** | sdk-python #59 (P1, ArcPy scanner), server #1096 (no priority label), site #9 (P1 proof hub) | **STRONG but uncoordinated** | Three load-bearing pieces in three repos with no shared release plan. #1096 needs a P1 label. |
...
| **AI Studio / ArcGIS Pro alternative** | server #892 (AI app builder, P2/Beta), portal #5 (app builder, P3/GA), portal #1/#6 (catalog/open-data, P3) | **UNDER-PRIORITIZED** | If this is "the Pro killer", it can't be P2/P3. |
...
| **QGIS plugin** | [honua-sdk-python#65](https://github.com/honua-io/honua-sdk-python/issues/65), [`honua-qgis-plugin`](https://github.com/honua-io/honua-qgis-plugin), [landing page](https://honua.io/qgis-plugin.html) | **LIVE PREVIEW** | Source repo and static page are live; GitHub Release ZIP, QGIS marketplace approval, screenshots, and demo video remain pending. |
...
| **FedRAMP / SOC 2 / CMMC** | server #352 | **MISMATCH (P4/GA)** | SLG market thesis requires P1; sitting at P4 means won't start in 2026. See §7. |
...
| **Production-stability for the staging proof chain** | server #965 (TLS), terraform #24 (VPC quota), devops #41, sdk-python #53, terraform #22/#23 (DR drills) | **CHAIN BLOCKED by #965** | One bug blocks the whole chain. |
| **First paid pilot + voice of customer** | sales #14, sales #47 | **EARLY** | Trigger milestone exists; signal does not. |

### Reading the scorecard

**Strong areas:** marketplace launch, migration-tool components (uncoordinated), platform stability (chain blocked).

**Strategic gaps:** open-weights model, QGIS plugin, NIM, GovCloud, compliance. Appendix C records the filed issues and later execution updates for the gaps that have moved.
...
1. **Promote honua-server #965 to P0/stop-the-line; fix this week.** The cert misconfig blocks `api.honua.io` and `staging-api.honua.io` from presenting valid TLS, which blocks SDK staging smoke, terraform live validation, and the cloud-staging proof that the marketplace listings need. Single most impactful unblock.
2. **Add a priority label to honua-server #1096** (ArcGIS Pro licensed evidence workflow) — promote to P1 with a 30-day deadline. This is the foundation of the migration narrative.
3. **File the five strategic-gap epics** (see §5 for shapes):
...
4. **Promote honua-server #892** (AI app builder contract) from P2 to P1.
5. **File the FedRAMP / SOC 2 readiness epic** — see §7 for shape.
6. **Group enterprise GA wishlist** under one umbrella issue "Enterprise GA — post-first-pilot" with explicit do-not-start criteria; remove individual P4 issues from standing review.
...
3. **Ship the QGIS plugin v0** — local LLM via Ollama with Honua-GIS model preference, bounded PyQGIS query bridge, static landing page, release ZIP, and QGIS marketplace path.
...
**Problem:** honua-server #352 ("Compliance framework: SOC 2 / FedRAMP evidence collection, data residency, key rotation") is labeled **P4/GA**. Our stated GTM thesis targets state and local government, eventual GovCloud, eventual federal-touching SLG customers. These buyers ask "do you have SOC 2?" on the first call, and the procurement officer's questionnaire requires either SOC 2 attestation or FedRAMP-equivalent controls before evaluating.

**Why P4/GA is incompatible:**
...
- Sitting at P4 means it doesn't start in 2026. We will be having "we'll have it soon" conversations 12+ months from now.

**Recommendation:** file a new P1 epic this week with two repo touch-points:
...
**Specific sub-issue list to file under the umbrella:**
...
## Appendix A — Top issues by repo

(Compact list for reference; full data in inventory scan output.)

### honua-server (33 open) — heaviest backlog

Active P1s:
- **#965** TLS cert bug on api/staging hosts — **should be P0**
- **#1035** Metadata v2 canonical resource graph

Migration-relevant:
- **#1096** ArcGIS Pro licensed evidence workflow — needs P1 label
- **#892** AI app builder platform contract — promote P2 → P1
...
Compliance:
- **#352** Compliance framework — **promote P4 → P1**
...
- **#30** Deploy seeded cloud demo services — P1, blocked on server #965
...
**Landing page:** <https://honua.io/qgis-plugin.html>
**Current status:** `0.1.0` early preview; source and packaging scripts are live. GitHub Release ZIP, QGIS marketplace approval, production screenshots, and demo video remain pending.

**Sub-issues:**
1. Plugin skeleton + `metadata.txt` + QGIS plugin manifest
2. Chat panel UI in QGIS dockwidget
3. Bounded PyQGIS query bridge with Honua-GIS model preference
4. Local Ollama detection + `qwen2.5-coder` fallback until Honua-GIS beta is published
5. Optional remote NIM endpoint + OpenAI-compatible fallback
6. Audit JSONL local logging (no telemetry by default)
7. QGIS plugin marketplace submission (manual release-owner step)
8. Documentation + example workflows, including the static landing page
9. Distribution outside QGIS marketplace through GitHub Release ZIP once published
10. Demo video for marketing, using only release-matched screenshots and sample data
...
- [ ] Confirm Phase 1 actions in §4 — especially treating honua-server #965 as P0
- [ ] Confirm the five strategic gaps in §5 should be filed as epics this week
- [ ] Decide first pilot pricing in §9
- [ ] Decide compliance investment commitment in §7
...
|---|---|---|
| [honua-server#965](https://github.com/honua-io/honua-server/issues/965) | Cloud API hosts present GitHub Pages TLS certificate | priority/P1 → **priority/P0**. The stop-the-line unblock. |
...
| [honua-server#892](https://github.com/honua-io/honua-server/issues/892) | Platform contract: AI app builder and spatial query demo enablement | priority/P2 → **priority/P1** + added **blocks-others**, **ready-to-start**. |
| [honua-server#352](https://github.com/honua-io/honua-server/issues/352) | Compliance framework: SOC 2 / FedRAMP evidence collection | priority/P4 → **priority/P1** + added **blocks-others**. (Note: phase remains GA; SOC 2 Type 2 is a 2027 milestone. Active execution coordinated from honua-sales#48.) |
...
### Founder decisions still required before agentflow proceeds

These match the §9 decision points and gate the new epics:

1. **honua-esri-assess repo location** (decision #1) — dedicated `honua-io/honua-esri-assess` repo (recommended) vs. sub-package in honua-sdk-python. Until decided, epic #62 cannot proceed past planning.
2. **honua-gis-llm repo location** (decision #2) — dedicated `honua-io/honua-gis-llm` repo (recommended) vs. sub-package. Until decided, epic #64 cannot proceed past planning.
3. **Honua-GIS-32B base model choice** (decision #3) — Qwen 2.5 Coder 32B (recommended; Apache-2.0) vs. Llama 3.3 70B (broader knowledge, more restrictive license). Affects epic #64 corpus + training pipeline design.
4. **honua-qgis-plugin repo creation** — resolved 2026-05-23. Dedicated `honua-io/honua-qgis-plugin` repo exists with GPL-2+ license, and the static landing page is live at <https://honua.io/qgis-plugin.html>. Remaining release-owner follow-ups are the GitHub Release ZIP, QGIS marketplace approval, screenshots, and demo video.
5. **Esri Partner Network confirmation** (decision #4) — confirm we do not join. No backlog action; document the call in honua-sales/docs/strategy/.
6. **Compliance investment commitment** (decision #5) — confirm ~$30-80K audit spend + 10-20% engineering capacity over 12 months. Gates honua-sales#48 from quote-collection to auditor-contracting.
7. **honua-mobile-sdk archive** (decision #6) — confirm. No backlog action filed yet; recommended.
...
- §1 Executive summary "biggest single ask" → [honua-server#965](https://github.com/honua-io/honua-server/issues/965) (P0)
...
- §5 Epic 2 QGIS plugin → [honua-sdk-python#65](https://github.com/honua-io/honua-sdk-python/issues/65), [honua-qgis-plugin](https://github.com/honua-io/honua-qgis-plugin), and [honua.io/qgis-plugin.html](https://honua.io/qgis-plugin.html)
