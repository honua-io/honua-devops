#!/usr/bin/env bash
# FAULT-015-verify-restored.sh - Verify OTEL exporter endpoint is restored
set -euo pipefail
...
echo "[FAULT-015] Verifying OTEL exporter endpoint is restored"
echo "  Environment: ${FAULT_ENV}"
echo "  Region:      ${FAULT_REGION}"
echo "  Service:     ${SERVICE_NAME}"

if [[ "${FAULT_DRY_RUN}" == "true" ]]; then
    echo "[DRY-RUN] Would verify OTEL_EXPORTER_OTLP_ENDPOINT no longer equals '${INVALID_OTEL_ENDPOINT}'"
    exit 0
fi
...
if [[ -n "${CURRENT_ENDPOINT}" && "${CURRENT_ENDPOINT}" != "${INVALID_OTEL_ENDPOINT}" && "${CURRENT_ENDPOINT}" != "None" ]]; then
    echo "[FAULT-015] Verified OTEL endpoint is restored: ${CURRENT_ENDPOINT}"
    exit 0
fi

echo "[FAULT-015] ERROR: OTEL endpoint is not restored (value: '${CURRENT_ENDPOINT}')" >&2
exit 1
