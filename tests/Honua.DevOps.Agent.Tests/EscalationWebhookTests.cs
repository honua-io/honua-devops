...
public class EscalationWebhookTests
{
    ...
    public void Verifier_RejectsMismatchedSignature()
    {
        ...
        byte[] body = Encoding.UTF8.GetBytes("{\"event\":\"ticket.escalation_requested\"}");
    ...
    public async Task Handler_Returns401OnBadSignature()
    {
        ...
        byte[] body = Encoding.UTF8.GetBytes(BuildValidPayloadJson());
    ...
    {
        string payload = """{"event":"ticket.created","ticketId":"T-1"}""";
    ...
    public async Task Handler_Returns202AndInvokesCallbackOnValidEvent()
    {
        ...
        Assert.Equal("Service returning 500 errors", captured.Symptoms);
        Assert.Equal("Connection pool exhausted", captured.DiagnosisSummary());
        Assert.Equal("operator-scoped", captured.DiagnosisMode());
    }
    ...
    private static string BuildValidPayloadJson()
    {
        object body = new
        {
            @event = "ticket.escalation_requested",
            ticketId = "ST-20260520-0042",
            customerId = "customer-x",
            severity = "critical",
            environment = "prod",
            service = "roads-api",
            symptoms = "Service returning 500 errors",
            diagnosis = new
            {
